...
export default function ClientLogin() {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    setLoading(true)
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL}/auth/client-login`, {
        method: 'POST',
      ...
      login(data.token, data.assignedProjects)
      if (data.assignedProjects.length === 1) {
        navigate(`/${data.assignedProjects[0]}`)
      } else {
        navigate('/portal')
      }
    } catch {
  ...
  return (
    <div className="min-h-screen bg-cream-100 flex flex-col items-center justify-center px-6">
      <div className="w-full max-w-sm">
        ...
        <h1 className="font-serif text-3xl text-ink mb-1">Client portal</h1>
        <p className="text-stone-500 text-sm mb-8">Sign in to view your project dashboard.</p>
